...
describe('', () => {
    it('transformArguments', () => {
        assert.deepEqual(
            ...
            ['GRAPH.CONSTRAINT', 'DROP', 'key', 'UNIQUE', 'NODE', 'label', 'PROPERTIES', '2', 'prop1', 'prop2']
        );
    });
    ...
    testUtils.testWithClient('client.graph.delete', async client => {
        ...
        assert.equal(
            await client.falkordb.constraintDrop('key', ConstraintType.UNIQUE, EntityType.NODE, 'label', 'prop1', 'prop2'),
            'ok',
        );
    }, GLOBAL.SERVERS.OPEN);
});
