...
describe("Single Client Tests", () => {
  ...
  describe("Basic Single Client Operations", () => {
    it(
      "should create single client instance",
      skipIfNoClient(() => {
    ...
    it(
      "should execute query through single client",
    ...
    it(
      "should execute read-only query through single client",
      skipIfNoClient(async () => {
    ...
    it(
      "should delete graph through single client",
      skipIfNoClient(async () => {
    ...
    it(
      "should explain query through single client",
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
          `test-single-explain-${getRandomNumber()}`
        );
    ...
    it(
      "should profile query through single client",
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
          `test-single-profile-${getRandomNumber()}`
        );
    ...
    it(
      "should list graphs through single client",
      skipIfNoClient(async () => {
        const graphs = await singleClient.list();
    ...
    it(
      "should get config through single client",
      skipIfNoClient(async () => {
    ...
    it(
      "should set config through single client",
      skipIfNoClient(async () => {
        ...
        const originalValue = Array.isArray(originalConfig)
          ? originalConfig[1]
          : originalConfig;
        await singleClient.configSet(
          "RESULTSET_SIZE",
    ...
    it(
      "should get info through single client",
      skipIfNoClient(async () => {
    ...
    it(
      "should get info with section through single client",
      skipIfNoClient(async () => {
    ...
    it(
      "should get memory usage through single client",
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
          `test-single-memoryusage-${getRandomNumber()}`
        );
    ...
    it(
      "should get slow log through single client",
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
    ...
    it(
      "should create and drop constraints through single client",
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
          `test-single-constraint-create-${getRandomNumber()}`
        );
        try {
    ...
    it(
      "should copy graphs through single client",
      skipIfNoClient(async () => {
    ...
    it(
      "should get connection from single client",
      skipIfNoClient(async () => {
  ...
  describe("Pooled Single Client Operations", () => {
    it(
      "should execute query through pooled single client",
      skipIfNoPooledClient(async () => {
        const graph = pooledClient.selectGraph(
          `test-pooled-query-${getRandomNumber()}`
        );
        const result = await graph.query("RETURN 1 as num");
        expect(result).toBeDefined();
        await graph.delete();
    ...
    it(
      "should execute read-only query through pooled single client",
      skipIfNoPooledClient(async () => {
        const graph = pooledClient.selectGraph(
          `test-pooled-ro-query-${getRandomNumber()}`
        );
        // First create some data
        await graph.query("CREATE (n:Test {value: 1})");
        // Then use roQuery to read it
    ...
    it(
      "should delete graph through pooled single client",
      skipIfNoPooledClient(async () => {
        const graph = pooledClient.selectGraph(
          `test-pooled-delete-${getRandomNumber()}`
        );
    ...
    it(
      "should explain query through pooled single client",
      skipIfNoPooledClient(async () => {
        const graph = pooledClient.selectGraph(
          `test-pooled-explain-${getRandomNumber()}`
        );
        // Create some data first for a meaningful explain
        await graph.query('CREATE (n:Person {name: "Alice"})');
        const result = await graph.explain("MATCH (n:Person) RETURN n");
    ...
    it(
      "should profile query through pooled single client",
      skipIfNoPooledClient(async () => {
        const graph = pooledClient.selectGraph(
          `test-pooled-profile-${getRandomNumber()}`
        );
        // Create some data first for a meaningful profile
        await graph.query('CREATE (n:Person {name: "Alice"})');
        const result = await graph.profile("MATCH (n:Person) RETURN n");
    ...
    it(
      "should get memory usage through pooled single client",
      skipIfNoPooledClient(async () => {
        const graph = pooledClient.selectGraph(
          `test-pooled-memoryusage-${getRandomNumber()}`
        );
        await graph.query("UNWIND range(1, 10) AS x CREATE (n:n)-[e:e]->(m:m)");
        const memoryProps = [
          "total_graph_sz_mb",
    ...
    it(
      "should get slow log through pooled single client",
      skipIfNoPooledClient(async () => {
        const graph = pooledClient.selectGraph(
          `test-pooled-slowlog-${getRandomNumber()}`
        );
    ...
    it(
      "should test pooled client with multiple concurrent operations",
      skipIfNoPooledClient(async () => {
        const operations = [
          pooledClient.info(),
          pooledClient.list(),
    ...
    it(
      "should test pooled client with graph operations",
      skipIfNoPooledClient(async () => {
        const graph1 = pooledClient.selectGraph(
          `pool-test-1-${getRandomNumber()}`
        );
        const graph2 = pooledClient.selectGraph(
          `pool-test-2-${getRandomNumber()}`
        );
  ...
  describe("Error Handling and Edge Cases", () => {
    it(
      "should handle invalid config operations",
      skipIfNoClient(async () => {
        await expect(
          singleClient.configGet("INVALID_CONFIG_KEY")
        ).rejects.toThrow();
        await expect(
          singleClient.configSet("INVALID_KEY", "invalid_value")
        ).rejects.toThrow();
      })
    );

    it(
      "should verify Single client methods exist",
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
          `method-test-${getRandomNumber()}`
        );
    ...
    it(
      ...
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
          `invalid-query-test-${getRandomNumber()}`
        );
    ...
    it(
      "should handle operations on deleted graphs",
      skipIfNoClient(async () => {
        const graph = singleClient.selectGraph(
          `deleted-graph-test-${getRandomNumber()}`
        );
